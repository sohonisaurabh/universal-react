 ...
 */
const checkDescriptor = (descriptor: { saga?: Function; mode?: string }) => {
  const shape = {
    saga: isFunction,
    mode: (mode?: string) => isString(mode) && allowedModes.indexOf(mode) > -1,
  };
...
export function injectSagaFactory(store: CustomStore, isValid: boolean) {
  ...
  function injectSaga(
    key: string,
    descriptor: { saga?: Function; mode?: string } = {},
    args?: Object
  ) {
